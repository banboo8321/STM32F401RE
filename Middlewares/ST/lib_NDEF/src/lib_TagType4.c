...
uint8_t Transmit_Data[DATA_MAX_SIZE]={0};
...
#if 0 /*NFC  config SSID¡¡and passward ;add	by jiangdong at 2/10*/
	uint8_t ssidData[SSID_LENGTH]={0x48,0x6F,0x6D,0x65,0x57,0x4C,0x41,0x4E};
 ...
 */
struct sRecordInfo RecordStruct;
...
/**
  ...
  */
uint16_t TT4_ReadWifiToken(sWifiTokenInfo *pWifiToken)
{
	uint16_t status = ERROR;
	sCCFileInfo *pCCFile;
	struct  sRecordInfo *pRecordStruct;
...
/**
  ...
  */
uint16_t TT4_WriteWifiToken(sWifiTokenInfo *pWifiToken)
{
	uint16_t status = ERROR;
	sCCFileInfo *pCCFile;
